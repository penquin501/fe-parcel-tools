  ...
  <div>
    ...
    <el-card style="width: 100%">
      ...
      <div style="margin-top: 20px;"></div>
        <el-form :inline="true" class="demo-form-inline" ref="form" :model="formMember" label-width="300px">
          <el-form-item label="กรุณาใส่รหัสสมาชิก(Member Id)">
            <el-input v-model="formMember.memberId" maxlength="30" clearable></el-input>
          </el-form-item>
          <el-form-item>
            <el-button icon="el-icon-search" @click.prevent="getMemberInfo">ค้นหา</el-button>
          </el-form-item>
        </el-form>
        <div v-show="memberInfo.memberId !== undefined">
          <el-descriptions title="ข้อมูลสมาชิก">
            <el-descriptions-item label="Member Id:">{{ memberInfo.memberId }}</el-descriptions-item>
            ...
            <el-descriptions-item label="ที่อยู่ผู้ส่ง">{{ memberInfo.refAddress }}</el-descriptions-item>
          </el-descriptions>
        </div>
    </el-card>
    <div style="margin-top: 20px;"></div>
...
export default defineComponent({
  ...
  methods: {
    ...
    getMemberInfo(this: any) {
      if (this.formMember.memberId.trim() == "") {
        this.$alert("กรุณาใส่รหัสผู้ส่งให้ถูกต้อง", "", { confirmButtonText: "OK" });
      } else if (this.formMember.memberId.trim() == this.billingInfo.member_code) {
        this.$alert("กรุณาใส่รหัสผู้ส่งให้ถูกต้อง เนื่องจากรหัสสมาชิกเป็นของบิลนี้อยู่แล้ว", "", { confirmButtonText: "OK" });
      } else {
        var data = {
          member_code: this.formMember.memberId.trim()
        };
        axios
          ...
          .then(response => {
            if (response.data.status == "SUCCESS") {
              ...
              this.memberInfo.phoneregis = 
                (this.memberInfo.phoneregis[0] + this.memberInfo.phoneregis[1]) =='66' ? 
                    this.changeDoubleSix2Zero(this.memberInfo.phoneregis) : 
                    this.memberInfo.phoneregis;
            } else {
              this.$alert("กรุณาใส่รหัสผู้ส่ง(member code) ให้ถูกต้อง", "", { confirmButtonText: "OK" });
              this.memberInfo = {};
            }
          })
          .catch(function(error) {
    ...
    confirmData(this:any) {
      if (this.billingInfo.billing_no == undefined) {
      ...
      } else if (this.memberInfo.memberId == "") {
        this.$alert("กรุณาเลือกผู้ส่งให้ถูกต้อง", "", { confirmButtonText: "OK" });
      } else if (this.memberInfo.memberId.trim() == this.billingInfo.member_code) {
        this.$alert("กรุณาเลือกผู้ส่งให้ถูกต้อง เนื่องจากรหัสสมาชิกเป็นของบิลนี้อยู่แล้ว", "", { confirmButtonText: "OK" });
        ...
        this.$alert("กรุณาเลือกผู้ส่งที่อยู่ในสาขาเดียวกันเท่านั้น", "", { confirmButtonText: "OK" });
        this.memberInfo = {};
      } else {
        ...
        var dataConfirm = {
          ...
          selectItem: [],
          currentMember: {
            memberCode: this.memberInfo.memberId,
            senderName: this.memberInfo.firstname + " " + this.memberInfo.lastname,
            senderPhone: this.memberInfo.phoneregis,
            senderAddress: this.memberInfo.refAddress
          },
