          ...
          <td style="width: 70%;">
              <el-descriptions :title='`ข้อมูล Tracking: ${billingInfo.tracking}`' :column="2">
                <el-descriptions-item label="ชื่อผู้รับ:">
                  <el-input @keypress="inputCheckFormatName($event)" v-model="billingInfo.receiver_name" readonly></el-input>
                </el-descriptions-item>
                <el-descriptions-item label="เบอร์โทรศัพท์ผู้รับ:">
                  <el-input @keypress="onlyNumber(ele, $event)" v-model="billingInfo.phone" maxlength="10" readonly></el-input>
                </el-descriptions-item>
                <el-descriptions-item label="ตำบล:">
                  <el-input :placeholder="billingInfo.district_name" v-model="district_name" readonly></el-input>
                </el-descriptions-item>
                <el-descriptions-item label="อำเภอ:">
                  <el-input :placeholder="billingInfo.amphur_name" v-model="amphur_name" readonly></el-input>
                </el-descriptions-item>
                <el-descriptions-item label="จังหวัด:">
                    <el-select style="width: 100%;" ref="branch" v-model="selectedProvince" placeholder="จังหวัด" @change="selectProvince()" disabled>
                ...
                <el-descriptions-item label="รหัสไปรษณีย์:">
                  <el-input maxlength="5" @keypress="onlyNumber(ele, $event)" v-model="zipcode" readonly></el-input>
                </el-descriptions-item>
                <el-descriptions-item label="ที่อยู่ผู้รับ:">
                  <el-input style="width: 100%;" v-model="billingInfo.receiver_address" readonly></el-input>
                </el-descriptions-item>
                <el-descriptions-item>
                    <span>
...
export default defineComponent({
  ...
  methods: {
    getData(this: any) {
      axios
        .get(this.url + "/check/info/tracking?tracking=" + this.$store.state.ninja_booking_tracking.tracking.toUpperCase())
        .then(response => {
          if (response.data.status == "SUCCESS") {
            this.$store.commit("ninjaCheckerPreviousValue", JSON.stringify(response.data.billingInfo[0])); //เก็บไว้ส่งค่าตอน confirm
            ...
            this.district_name = this.billingInfo.district_name;
            this.amphur_name = this.billingInfo.amphur_name;
            this.zipcode = this.billingInfo.br_zipcode;
    ...
    confirmData(this: any) {
      ...
      if (this.tracking == "") {
        ...
        var dataConfirm = {
          ...
          previous_value: JSON.parse(this.$store.state.ninja_previous_value),
          current_value: {
        ...
        axios
          ...
          .then(response => {
            if (response.data.status == "SUCCESS") {
                //@ts-ignore
                this.$store.commit("ninjaCheckerPreviousValueClear");
                this.$alert("ข้อมูลได้ update เข้าระบบแล้ว", "", { confirmButtonText: "OK" });
                this.$router.push({ name: "NinjaBookingChecker" });
            } else {
